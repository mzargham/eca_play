...
We consider a **discrete-time dynamical system** over the Boolean hypercube $\mathbb{B}^n = \{0,1\}^n$, defined by:

```math
z_{k+1} = f(z_k)
```

where $z_k \in \mathbb{B}^n$, and $f: \mathbb{B}^n \to\mathbb{B}^n$ is a **Boolean linear operator** (e.g., matrix multiplication over GF(2)).

### Elementary Cellular Automata as Boolean Linear Dynamical Systems

Each **Elementary Cellular Automaton (ECA)** can be described as a **Boolean discrete-time dynamical system** over a ring of $n$ binary cells. The state vector $z \in \mathbb{B}^n$ evolves by applying a local rule to each cell’s 3-bit neighborhood.

We model this using a **linear operator over the Boolean semiring** $(\mathbb{B}, \vee, \wedge)$, decomposed into three stages:
...
3. **Reduce**: Apply the Wolfram rule bits $b \in \mathbb{B}^8$ as a Boolean dot product to compute the next state.
...
```math
X_k = [z, df(z), d^2f(z), \ldots, d^kf(z)] \in \mathbb{B}^{(k+1)n}
```
...
In the **Boolean setting**, the state space is finite: $z \in \mathbb{B}^n$ and our Boolean derivitive operator d.

A **Boolean jet of order $k$** is:

```math
J^k(z) = [z, df(z), d^2f(z), \dots, d^kf(z)] \in \mathbb{B}^{(k+1)n}
```
...
1. **Initialize**: Start from every possible initial extended state $X_0 \in \mathbb{B}^{(k+1)n}$ or a sampled subset.
